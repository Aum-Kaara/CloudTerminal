...
namespace CloudSense
{
    public static class AzureResourceManagerUtil
    {
        ...
        {
            List<Subscription> _list = new List<Subscription>();
            var _org = GetUserOrganizations();
            foreach (Organization item in _org)
            {
                var sub = GetUserSubscriptions(item.Id);
                if (sub != null)
                {
                    _list.AddRange(sub);
                }
        ...
        public static string GetVirtualMachines(string subscriptionId)
        {
            ...
            try
            {
                ...
                // Get subscriptions to which the user has some kind of access
                string requestUrl = string.Format("{0}/subscriptions/{1}/resources?api-version=2015-11-01", ConfigurationManager.AppSettings["ida:AzureResourceManagerUrl"],
                    subscriptionId);
        ...
        public static string GetVMInfo(string Id)
        {
            ...
            try
            {
                ...
                // Get subscriptions to which the user has some kind of access
                string requestUrl = string.Format("{0}/{1}?api-version=2016-04-01", ConfigurationManager.AppSettings["ida:AzureResourceManagerUrl"],
                    Id);
        ...
        public static string CallAPI(string requestUrl, string ApiType = "GET")
        {
            ...
            catch { throw new Exception("Exception Occur while calling API"); }
